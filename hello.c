...
void save_progress_to_file(const char *username);
    ...
    while (1) {
        ...
        case 2:
            ...
            if (user_index >= 0) {
                ...
                // save_progress_to_file(users[user_index].username);  // Save the progress to user's file
                system("type.exe");
            } else {
...
void save_progress_to_file(const char *username) {
    char filename[50];
    snprintf(filename, sizeof(filename), "%s_progress.txt", username);

    FILE *file = fopen(filename, "a"); // Open in append mode
    if (file) {
        // Here, we capture the output of type.exe and write it to the file.
        FILE *exe_output = popen("type.exe", "r");
        if (exe_output) {
            char buffer[256];
            while (fgets(buffer, sizeof(buffer), exe_output) != NULL) {
                fprintf(file, "%s", buffer); // Write the output to the user-specific file
            }
            fclose(exe_output);
        }
        fclose(file);
        ...
        printf("Failed to open progress file for writing.\n");
    }
}
